...
func TestTransferCmdResponse(t *testing.T) {
    eofByte := []byte{
        4,0,0,0,
        0xfe,0,0,0,
    }
    ...
    client := NewBufferConn(make([]byte, 1024), cmdByte)
    serverResp := append(resByte, eofByte...)
    serverResp = append(serverResp, resByte...)
    serverResp = append(serverResp, eofByte...)
    server := NewBufferConn(make([]byte, 1024), serverResp)

    serverConn := NewConn(server)
    clientConn := NewConn(client)

    err := serverConn.TransportCmdResp(clientConn)
    assert.Nil(t, err, "TransportCmdResp err")
}

func TestTransferCmdResponse2(t *testing.T) {
    okBytes := []byte{
        4,0,0,0,
        0,0,0,0,
    }
    ...
    client := NewBufferConn(make([]byte, 1024), cmdByte)
    server := NewBufferConn(make([]byte, 1024), okBytes)

    serverConn := NewConn(server)
    clientConn := NewConn(client)

    err := serverConn.TransportCmdResp(clientConn)
    assert.Nil(t, err, "TransportCmdResp2 err")
}

func TestTransferCmdResponse3(t *testing.T) {
    errByte := []byte{
        4,0,0,0,
        0xff,0,0,0,
    }
    ...
    client := NewBufferConn(make([]byte, 1024), cmdByte)
    server := NewBufferConn(make([]byte, 1024), errByte)

    serverConn := NewConn(server)
    clientConn := NewConn(client)

    err := serverConn.TransportCmdResp(clientConn)
    assert.Nil(t, err, "TransportCmdResp3 err")
}
